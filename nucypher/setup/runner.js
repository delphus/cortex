...
const startAlice = alicePort => {
  return new Promise((resolve, reject) => {
    const handle = exec(
      `nucypher alice run --teacher ${URSULA_URL} --dev --federated-only --controller-port ${alicePort}`,
      (err, stdout, stderr) => {
    ...
    handle.stdout.on("data", data => {
      ...
      if (data.includes("Encrypting")) {
        config["Alice"][alicePort - 3000].encryption_key = data.match(
          /[0-f]{66}/g
        )[0];
      } else if (data.includes("Verifying")) {
        config["Alice"][alicePort - 3000].verification_key = data.match(
          /[0-f]{66}/g
        )[0];
      }
    });
...
const startBob = bobPort => {
  return new Promise((resolve, reject) => {
    const handle = exec(
      `nucypher bob run --teacher ${URSULA_URL} --dev --federated-only --controller-port ${bobPort}`,
      (err, stdout, stderr) => {
    ...
    handle.stdout.on("data", data => {
      ...
      if (data.includes("Encrypting")) {
        config["Bob"][bobPort - 4000].encryption_key = data.match(
          /[0-f]{66}/g
        )[0];
      } else if (data.includes("Verifying")) {
        config["Bob"][bobPort - 4000].verification_key = data.match(
          /[0-f]{66}/g
        )[0];
      }
    });
...
const startEnrico = (enricoPort, policyEncryptingKey) => {
  return new Promise((resolve, reject) => {
    const handle = exec(
      `nucypher enrico run --http-port ${enricoPort} --policy-encrypting-key ${policyEncryptingKey}`,
      (err, stdout, stderr) => {
    ...
    handle.stdout.on("data", data => {
      ...
      if (data.includes("Encrypting")) {
        config["Enrico"][enricoPort - 5000].encryption_key = data.match(
          /[0-f]{66}/g
        )[0];
      } else if (data.includes("Verifying")) {
        config["Enrico"][enricoPort - 5000].verification_key = data.match(
          /[0-f]{66}/g
        )[0];
      }
    });
  ...
  return new Promise((resolve, reject) => {
    setTimeout(async () => {
      const resp = await http.post(
        `${aliceUrl}/derive_policy_encrypting_key/${policyName}`
      );
...
const setPolicy = policyName => {
  ...
  return util.grantAccess(
    config["Bob"][0].encryption_key,
    config["Bob"][0].verification_key,
    policyName,
    1,
    1,
    "2020-09-15T15:20:00"
