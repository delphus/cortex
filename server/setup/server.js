...
var serviceAccount = require("./a.json.js");
...
app.post("/retrieve", async function(req, res) {
  ...
  const msg = await util.retreiveMessage(
    parseInt(bobIndex),
    ALICE_VERIFYING_KEY,
...
app.post("/grant", async (req, res) => {
  ...
  const msg = await util.grantAccess(
    bob_encrypting_key,
    BOB_VERIFYING_KEYS[bobIndex],
    policyName,
    m,
    n,
    expiry
